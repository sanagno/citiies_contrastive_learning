...
import augmentations_temporal as aug
...
def main(args):
    ...
    loader = DataLoader(dataset=dataset, batch_size = per_device_batch_size, num_workers=args.num_workers,  sampler = sampler)     #sampler=sampler)
    val_loader = DataLoader(dataset=val_dataset, batch_size = per_device_batch_size, num_workers=args.num_workers, sampler = sampler) #sampler=sampler)
    ...
    if args.rank == 0:
        torch.save(model.module.backbone.state_dict(), args.exp_dir / "resnet50_vicreg_sn7_temporal_100.pth")
...
class SN7Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        contain_values = self.img_ids_labels[self.img_ids_labels[0].str.contains(location_id) & self.img_ids_labels[0].str.startswith(patch_id+'!')] 
        #print(image_id)
        #print(location_id)
        #print(patch_id)
        #print(contain_values)
        
        image_2_full = contain_values.sample().iloc[0, 0]
        #print('image2:',image_2_full)
        image_id_2, patch_id_2 = image_2_full.split("!")[1],image_2_full.split("!")[0]
        pattern_2 = "mosaic_(.*?).tif"
        location_id_2 = re.search(pattern_2, image_id_2).group(1)
        ...
        img_path_2 = os.path.join(self.img_dir, location_id_2, 'images', image_id_2 )
        image_2 = torchvision.transforms.ToTensor()(Image.open(img_path_2))[0:3,:,:]  #TAKE RGB CHANNELS ONLY FOR RESNET COMPATIBILITY!!!
        
        image_padded_2 = torch.nn.functional.pad(image_2, pad=(0, 1024 - image_2.shape[2], 0, 1024 - image_2.shape[1]))
        patches_2 = image_padded_2.unfold(1, 256, 256).unfold(2, 256, 256)
        patches_2 = patches_2.reshape(3, -1, 256, 256)
        patches_2 = patches_2.permute(1,0,2,3)
        
        image_2 = patches_2[int(patch_id_2)]
        
        image1, image2 = self.transform(image, image_2)
